...
cd ~
git clone https://github.com/zhiyuanep/nodejs-pool.git  # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
cd /usr/local/src
sudo git clone https://github.com/sumokoin-project/sumokoin.git
cd sumokoin
sudo git checkout v0.2.0.0
curl https://raw.githubusercontent.com/zhiyuanep/nodejs-pool/master/deployment/monero_daemon.patch | sudo git apply -v
sudo make -j$(nproc)
sudo cp ~/nodejs-pool/deployment/sumokoin.service /lib/systemd/system/
sudo useradd -m sumokoindaemon -d /home/sumokoindaemon
BLOCKCHAIN_DOWNLOAD_DIR=$(sudo -u sumokoindaemon mktemp -d)
sudo -u sumokoindaemon wget --limit-rate=50m -O $BLOCKCHAIN_DOWNLOAD_DIR/blockchain.raw https://cloud.stty.me/blockchain.raw
sudo -u sumokoindaemon /usr/local/src/sumokoin/build/release/bin/sumokoin-blockchain-import --input-file $BLOCKCHAIN_DOWNLOAD_DIR/blockchain.raw --batch-size 20000 --database lmdb#fastest --verify off --data-dir /home/sumokoindaemon/.bitsumokoin
sudo -u sumokoindaemon rm -rf $BLOCKCHAIN_DOWNLOAD_DIR
